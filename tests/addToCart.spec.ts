...
test.describe("Add to cart test", () => {
  ...
  test("should add two products to cart and remove one", async ({
    productsPage,
    cartPage,
    allure,
  }) => {
    ...
    await productsPage.validateRemoveButtonText(
      productData.product.bikeLight.id,
    );
    await productsPage.validateAddToCart("2");
    await productsPage.goToCart();
    await cartPage.validateCartInventory(
      productData.screenshots.cartTwoProducts,
    );
    await cartPage.clickContinueShopping();
    await productsPage.removeFromCart(productData.product.backpack.id);
    await productsPage.validateAddButtonText(productData.product.backpack.id);
